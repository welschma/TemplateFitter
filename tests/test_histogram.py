...
from templatefitter import Histogram, Hist1d, Hist2d
...
class TestHist1d(unittest.TestCase):
    ...
    def setUp(self):
        self.test_data = np.array([0.2, 0.3, 0.2, 0.7,
                                   1.2, 1.5, 1.3, 1.3,
                                   2.4, 2.6, 2.2])
        self.test_weights = np.array([1, 2, 1, 1,
                                      2, 2, 2, 1,
                                      1, 2, 2])
        self.test_bin_counts = np.array([5., 7., 5.])
        self.test_bin_errors = np.sqrt(np.array([7., 13., 9.]))
    ...
    def test_init_with_data_and_weights(self):
        ...
        np.testing.assert_array_equal(hist.bin_counts, self.test_bin_counts)
        np.testing.assert_array_equal(hist.bin_errors, self.test_bin_errors)
        np.testing.assert_array_equal(hist.bin_edges, np.array([0., 1., 2., 3.]))
...
class TestHist2d(unittest.TestCase):
    ...
    def test_init_with_data_weights_and_bin_edges(self):
        ...
        np.testing.assert_array_equal(hist.x_projection()[1], np.sqrt(np.array([14, 17])))
        np.testing.assert_array_equal(hist.y_projection()[1], np.sqrt(np.array([26, 5])))

        hist.fill(np.array([[0.4, 4.5]]), np.array([3]))

        new_bin_counts = self.test_bin_counts + np.array([[0, 3], [0, 0]])
        new_bin_errors = np.sqrt(self.test_bin_errors**2 + np.array([[0, 3], [0, 0]])**2)
        ...
        np.testing.assert_array_equal(hist.x_projection()[1], np.sqrt(np.array([23, 17])))
        np.testing.assert_array_equal(hist.y_projection()[1], np.sqrt(np.array([26, 14])))
